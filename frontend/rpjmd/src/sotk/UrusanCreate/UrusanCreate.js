...
class UrusanCreate extends Component {
  ...
  handleChange(event){
    ...
    if(parseInt(no)===1){
      setData.Nm_Urusan = event.target.value;
    }
...
/// set data
  ...
  getData = (page = 1) => {
    ...
    axios
    .post(config.apiRoot+'sotk/urusan/page-'+page, data)
    .then(response => {
...
// submit
  handleSubmit = event => {
    ...
    if(this.state.aksi === 'Edit'){
      link = config.apiRoot+'sotk/urusan/update';
      
    }else if(this.state.aksi === 'Tambah'){
      link = config.apiRoot+'sotk/urusan/create';
    }
    // console.log(this.state.dataPilih.visi_id);
    const head = { headers: { 'Content-Type': 'multipart/form-data' } };
    let data = new FormData();
    data.append('session', localStorage.getItem('codexv-session'));
    data.append('id', this.state.dataPilih.Kd_Urusan);
    data.append('urusan', this.state.dataPilih.Nm_Urusan);
    axios
    .post(link, data, head)
    .then(response => {
  ...
  handleDelete = () =>{
    ...
    data.append('id', this.state.dataPilih.Kd_Urusan);
    axios
    .post(config.apiRoot+'sotk/urusan/delete', data)
    .then(response => {
      if(response.data.status){
        this.modalDelete();
        this.getData();
                  ...
                  <Form method="post"  onSubmit={this.handleSubmit} className="form-horizontal" id="form-data">
                    <ModalBody>
                      <FormGroup row>
                        ...
                        <Col xs="12" md="9">
                          <Input type="text" id="visi" data-number="1" placeholder="" onChange={this.handleChange} value={this.state.dataPilih.Nm_Urusan} required autoFocus/>
