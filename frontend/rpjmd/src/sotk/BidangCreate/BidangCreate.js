...
class BidangCreate extends Component {
  ...
  constructor(props) {
    ...
    document.title = "Menyusun Bidang";
  ...
  handleChange(event){
    ...
    if(no===1){
      setData.Nm_Bidang = event.target.value;
    }else if(no===2){
      setData.Kd_Urusan = event.target.value;
    }else if(no===3){
      setData.Kd_Fungsi = event.target.value;
    }
...
/// set data
  ...
  setData = (dataAll) => {
    if(dataAll.status){
      this.setState({ dataAll: dataAll.data, jumlahPage: dataAll.jumlahPage, jumlahAll: dataAll.jumlahAll, dataKategori:dataAll.kategori, dataUrusan:dataAll.dataUrusan, dataFungsi:dataAll.dataFungsi});
    }
  }
  ...
  getData = (page = 1) => {
    ...
    axios
    .post(config.apiRoot+'sotk/bidang/page-'+page, data)
    .then(response => {
...
// submit
  handleSubmit = event => {
    ...
    if(this.state.aksi === 'Edit'){
      link = config.apiRoot+'sotk/bidang/update';
      
    }else if(this.state.aksi === 'Tambah'){
      link = config.apiRoot+'sotk/bidang/create';
    }
    // console.log(this.state.dataPilih.visi_id);
    const head = { headers: { 'Content-Type': 'multipart/form-data' } };
    let data = new FormData();
    data.append('session', localStorage.getItem('codexv-session'));
    data.append('id', this.state.dataPilih.Kd_Bidang);
    data.append('urusan', this.state.dataPilih.Kd_Urusan);
    data.append('bidang', this.state.dataPilih.Nm_Bidang);
    data.append('fungsi', this.state.dataPilih.Kd_Fungsi);
    axios
    .post(link, data, head)
    .then(response => {
  ...
  handleDelete = () =>{
    ...
    data.append('urusan', this.state.dataPilih.Kd_Urusan);
    axios
    .post(config.apiRoot+'sotk/bidang/delete', data)
    .then(response => {
      if(response.data.status){
        this.modalDelete();
        this.getData();
                  ...
                  <Form method="post"  onSubmit={this.handleSubmit} className="form-horizontal" id="form-data">
                    <ModalBody>
                      <FormGroup row>
                        ...
                        <Col xs="12" md="9">
                          <Input type="select" data-number="2" placeholder="" onChange={this.handleChange} value={this.state.dataPilih.Kd_Urusan} required autoFocus>
                      ...
                      <FormGroup row>
                        ...
                        <Col xs="12" md="9">
                          <Input type="text" id="visi" data-number="1" placeholder="" onChange={this.handleChange} value={this.state.dataPilih.Nm_Bidang} required autoFocus/>
                      ...
                      <FormGroup row>
                        ...
                        <Col xs="12" md="9">
                          <Input type="select" data-number="3" placeholder="" onChange={this.handleChange} value={this.state.dataPilih.Kd_Fungsi} required autoFocus>
                          <option key="-1" value="">-= Pilih Fungsi =-</option>
                          {this.state.dataFungsi.map((data, key) => {
                            return data ? (
                              <option key={key} value={data.Kd_Fungsi}>{data.Nm_Fungsi}</option>
                            ) : (null);
                          })}
                          </Input>
                          <FormText color="muted">Pilih Fungsi</FormText>
                        </Col>
                      </FormGroup>
