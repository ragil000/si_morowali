...
import { Alert, FormGroup, Input, CardBody, Col, Pagination, PaginationItem, PaginationLink, Row, Table, Modal, ModalBody, ModalFooter, ModalHeader, Button, Form, } from 'reactstrap';
...
class VisiPenjelasanCreate extends Component {
  
  constructor(props) {
    super(props);

    this.dataPilihAwal = {
      opd_visi_penjelasan_id:0,
      opd_visi_id:0,
      opd_visi_penjelasan_nama:'',
    };
    ...
    this.link = 'opd/menyusun/visi-penjelasan';
  ...
  handleChange = idx => evt => {
    ...
    if(no === 1){
      const newdataTests = this.state.dataAll.map((shareholder, sidx) => {
        ...
        let data = { ...shareholder, opd_visi_penjelasan_nama: evt.target.value };
...
// submit
  ...
  handleSubmit = () => {
    ...
    data.append('opd_visi_penjelasan_id', this.state.dataPilih.opd_visi_penjelasan_id);
    data.append('opd_visi_id', this.state.dataPilih.opd_visi_id);
    data.append('opd_visi_penjelasan_nama', this.state.dataPilih.opd_visi_penjelasan_nama);
...
edit(data){
  ...
  this.setState({dataPilih:this.dataPilihAwal, edit:data.opd_visi_penjelasan_id});
}
    ...
    if(this.state.loading){
      ...
      return(
        <div>
          ...
          <Row>
            <Col xs="128" md="10">
            <Download link="opd/menyusun/visi-penjelasan"/>
          ...
          <thead style={{textAlign: 'center', backgroundColor:'#0066ff', color: 'white'}}>
            ...
            {this.state.dataAll.map((data, key) => {
              if(data.opd_visi_penjelasan_id !== this.state.edit){
                return data ? (
                  <tr key={key}>
                    <td>{((this.state.page-1)*20)+key+1}</td>
                    <td>{data.opd_visi_penjelasan_nama}</td>
                    <td>
                ...
                return data ? (
                  <tr key={key}>
                    <td>{((this.state.page-1)*20)+key+1}</td>
                    <td><Input type="text" value={data.opd_visi_penjelasan_nama} data-number="1" onChange={this.handleChange(key)}/></td>
                    <td>
        ...
        <Row>
          <Col xs="12" lg="12">
              <CardBody>
                ...
                <h3 style={{textAlign: 'center',}}>Visi Penjelasan OPD</h3>
                <hr/>
                {this.isi()}
              </CardBody>
          </Col>
...
export default VisiPenjelasanCreate;
