...
import VisiPenjelasanCreate from './VisiPenjelasanCreate';
...
class VisiCreate extends Component {
  ...
  constructor(props) {
    ...
    document.title = "Menyusun Visi OPD";
  ...
  handleChange = idx => evt => {
    ...
    if(no === 1){
      const newdataTests = this.state.dataAll.map((shareholder, sidx) => {
        ...
        let data = { ...shareholder, opd_visi_nama: evt.target.value };
...
/// set data
  ...
  getData = (page = 1) => {
    ...
    axios
    .post(config.apiRoot+'opd/menyusun/visi/page-'+page, data)
    .then(response => {
...
// submit
  ...
  handleSubmit = () => {
    ...
    if(this.state.aksi === 'Edit'){
      link = config.apiRoot+'opd/menyusun/visi/update';
      
    }else if(this.state.aksi === 'Tambah'){
      link = config.apiRoot+'opd/menyusun/visi/create';
    }
    ...
    data.append('opd_visi_nama', this.state.dataPilih.opd_visi_nama);
  ...
  handleDelete = () =>{
    ...
    data.append('opd_visi_id', this.state.dataPilih.opd_visi_id);
    axios
    .post(config.apiRoot+'opd/menyusun/visi/delete', data)
    .then(response => {
      if(response.data.status){
        this.modalDelete();
        this.getData(this.state.page);
...
edit(data){
  ...
  this.setState({dataPilih:this.dataPilihAwal, edit:data.opd_visi_id});
  // console.log(this.state.dataPilih);
}
  ...
  isi(){
    if(this.state.loading){
      ...
      return(
        <div>
          {this.state.pesan}
          <Download link="opd/menyusun/visi"/>
          ...
          <thead style={{textAlign: 'center', backgroundColor:'#0066ff', color: 'white'}}>
            ...
            {this.state.dataAll.map((data, key) => {
              if(data.opd_visi_id !== this.state.edit){
                return data ? (
                  <tr key={key}>
                ...
                return data ? (
                  <tr key={key}>
                    <td>{((this.state.page-1)*20)+key+1}</td>
                    <td><Input type="text" value={data.opd_visi_nama} data-number="1" onChange={this.handleChange(key)}/></td>
                    <td>
          ...
          <Col xs="12" lg="12">
            <Card>
              ...
              <CardBody>
                ...
                <h3 style={{textAlign: 'center',}}>Visi OPD</h3>
                <hr/>
                {this.isi()}
                
              </CardBody>
              <VisiPenjelasanCreate/>
            </Card>
