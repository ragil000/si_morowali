...
class TujuanCreate extends Component {
  ...
  constructor(props) {
    ...
    document.title = "Menyusun Tujuan OPD";
    this.link = 'opd/menyusun/tujuan';
  ...
  handleChange = idx => evt => {
    ...
    if(no === 1){
      const newdataTests = this.state.dataAll.map((shareholder, sidx) => {
        ...
        let data = { ...shareholder, opd_misi_id: evt.target.value };
    ...
    if(no === 2){
      const newdataTests = this.state.dataAll.map((shareholder, sidx) => {
        if (idx !== sidx) return shareholder;
        let data = { ...shareholder, opd_tujuan_nama: evt.target.value };
        this.setState({dataPilih:data});
        return data;
      });
      this.setState({ dataAll: newdataTests });
...
// submit
  ...
  handleSubmit = () => {
    ...
    data.append('opd_tujuan_nama', this.state.dataPilih.opd_tujuan_nama);
...
edit(data){
  ...
  this.setState({dataPilih:this.dataPilihAwal, edit:data.opd_tujuan_id});
}
    ...
    if(this.state.loading){
      ...
      return(
        <div>
          ...
          <Row>
            <Col xs="128" md="10">
            <Download link="opd/menyusun/tujuan"/>
          ...
          <thead style={{textAlign: 'center', backgroundColor:'#0066ff', color: 'white'}}>
            ...
            {this.state.dataAll.map((data, key) => {
              if(data.opd_tujuan_id !== this.state.edit){
                return data ? (
                  <tr key={key}>
                ...
                return data ? (
                  ...
                  <tr key={key}>
                    <td>{((this.state.page-1)*20)+key+1}</td>
                    <td>
                      <Input type="select" value={data.opd_misi_id} data-number="1" onChange={this.handleChange(key)} required autoFocus >
                        <option key="-1" value="">-= Pilih Misi =-</option>
                        {this.state.dataTambah.map((data, key) => {
                          return data ? (
                            <option key={key} value={data.opd_misi_id}>{data.opd_misi_nama}</option>
                          ) : (null);
                        })}
                      </Input>
                    </td>
                    <td><Input type="text" value={data.opd_tujuan_nama} data-number="2" onChange={this.handleChange(key)}/></td>
                    <td>
