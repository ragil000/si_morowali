...
class SasaranCreate extends Component {
  ...
  constructor(props) {
    ...
    document.title = "Menyusun Sasaran OPD";
    this.link = 'opd/menyusun/sasaran';
  ...
  handleChange = idx => evt => {
    ...
    if(no === 1){
      const newdataTests = this.state.dataAll.map((shareholder, sidx) => {
        ...
        let data = { ...shareholder, opd_tujuan_id: evt.target.value };
    ...
    if(no === 2){
      const newdataTests = this.state.dataAll.map((shareholder, sidx) => {
        if (idx !== sidx) return shareholder;
        let data = { ...shareholder, opd_sasaran_nama: evt.target.value };
        this.setState({dataPilih:data});
        return data;
      });
      this.setState({ dataAll: newdataTests });
...
// submit
  ...
  handleSubmit = () => {
    ...
    data.append('opd_tujuan_id', this.state.dataPilih.opd_tujuan_id);
    data.append('opd_sasaran_nama', this.state.dataPilih.opd_sasaran_nama);
...
edit(data){
  ...
  this.setState({dataPilih:this.dataPilihAwal, edit:data.opd_sasaran_id});
}
    ...
    if(this.state.loading){
      ...
      return(
        <div>
          ...
          <Row>
            <Col xs="128" md="10">
            <Download link="opd/menyusun/sasaran"/>
          ...
          <thead style={{textAlign: 'center', backgroundColor:'#0066ff', color: 'white'}}>
            ...
            {this.state.dataAll.map((data, key) => {
              if(data.opd_sasaran_id !== this.state.edit){
                return data ? (
                  <tr key={key}>
                    <td>{((this.state.page-1)*20)+key+1}</td>
                    <td>{data.opd_tujuan_nama}</td>
                ...
                return data ? (
                  ...
                  <tr key={key}>
                    <td>{((this.state.page-1)*20)+key+1}</td>
                    <td>
                      <Input type="select" value={data.opd_tujuan_id} data-number="1" onChange={this.handleChange(key)} required autoFocus >
                        <option key="-1" value="">-= Pilih Misi =-</option>
                        {this.state.dataTambah.map((data, key) => {
                          return data ? (
                            <option key={key} value={data.opd_tujuan_id}>{data.opd_tujuan_nama}</option>
                          ) : (null);
                        })}
                      </Input>
                    </td>
                    <td><Input type="text" value={data.opd_sasaran_nama} data-number="2" onChange={this.handleChange(key)}/></td>
                    <td>
