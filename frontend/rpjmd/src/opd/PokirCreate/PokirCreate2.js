...
class PokirCreate extends Component {
...
/// set data
  ...
  getKegiatan = (urusan, bidang, program) => {
    console.log(urusan+"-"+bidang+"-"+program);
    this.setState({ loading: true });
    let data = new FormData();
    data.append('session', localStorage.getItem('codexv-session'));
    data.append('urusan', urusan);
...
downloadGambar = (data) =>{
  ...
  return (
    <Form method="post" id="download-gambar" action={config.apiMusrenbang+'attachments/foto-'+asal+'/'+data.file} target="_blank">
      <img alt="gambar-pokir" type="submit" form="download-gambar" src={config.apiMusrenbang+'attachments/foto-'+asal+'/'+data.file} style={{height:'100px'}} /><br/>
      <Input  type="hidden" name="session" value={localStorage.getItem('codexv-session')}/>
      <Button color="success"><i className="fa fa-file-photo-o"> Lihat</i></Button>
    </Form>
  );
}
...
keterangan(data, key){
  ...
  if(set){
    return (
      <div>
        <Input type="select" value={data.Kd_Prog} data-number="2" onChange={this.handleChange(key)}>
        ...
        <Input type="select" value={data.Kd_Keg} data-number="3" onChange={this.handleChange(key)}>
