...
class MusrenbangCreate extends Component {
  ...
  handleChange = idx => evt => {
    ...
    if(no === 3){
      ...
      console.log(pilih);
      this.getRkpd(this.state.dataPilih.Kd_Urusan, this.state.dataPilih.Kd_Bidang, this.state.dataPilih.Kd_Prog, pilih);
      const newdataTests = this.state.dataAll.map((shareholder, sidx) => {
        if (idx !== sidx) return shareholder;
        let data = { ...shareholder, Kd_Keg: pilih };
        this.setState({dataPilih:data});
        return data;
      });
      this.setState({ dataAll: newdataTests });
...
/// set data
  ...
  getData = (page = 1) => {
    ...
    .then(response => {
      ...
      axios
      .post(config.apiMusrenbang+'api/musrenbang/kiriman-pokir/page-'+page, data)
      .then(response => {
        this.setData(response.data)
        console.log(response);
