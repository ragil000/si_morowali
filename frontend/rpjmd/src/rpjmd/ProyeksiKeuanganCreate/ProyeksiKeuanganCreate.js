...
import { FormText, Alert, FormGroup, Label, Input, Card, CardBody, Col, Pagination, PaginationItem, PaginationLink, Row, Table, Modal, ModalBody, ModalFooter, ModalHeader, Button, Form, } from 'reactstrap';
import axios from 'axios';
import config from '../../config';
import loadingImage from '../../assets/img/loading.gif';
import AnalisisKeuanganCreate from './AnalisisKeuanganCreate';
import PaguIndikatifCreate from './PaguIndikatifCreate';
import Download from '../other/Download';


class ProyeksiKeuanganCreate extends Component {
  
  constructor(props) {
    super(props);

    this.dataPilihAwal = {
      proyeksi_keuangan_id:'',
      rekening:0,
      tahun1:0,
      tahun2:0,
      tahun3:0,
    ...
    document.title = "PROYEKSI KEMAMPUAN KEUANGAN DAERAH";
...
/// set data
  ...
  getData = (page = 1) => {
    ...
    axios
    .post(config.apiRoot+'rpjmd/menyusun/proyeksi-keuangan/page-'+page, data)
    .then(response => {
...
// submit
  handleSubmit = event => {
    ...
    if(this.state.aksi === 'Edit'){
      link = config.apiRoot+'rpjmd/menyusun/proyeksi-keuangan/update';
      
    }else if(this.state.aksi === 'Tambah'){
      link = config.apiRoot+'rpjmd/menyusun/proyeksi-keuangan/create';
    }
    ...
    data.append('session', localStorage.getItem('codexv-session'));
    data.append('rpjmd', localStorage.getItem('codexv-rpjmd'));
    data.append('proyeksi_keuangan_id', this.state.dataPilih.proyeksi_keuangan_id);
  ...
  handleDelete = () =>{
    ...
    data.append('proyeksi_keuangan_id', this.state.dataPilih.proyeksi_keuangan_id);
    axios
    .post(config.apiRoot+'rpjmd/menyusun/proyeksi-keuangan/delete', data)
    .then(response => {
      if(response.data.status){
        this.modalDelete();
        this.getData();
    ...
    if(this.state.loading){
      ...
      return(
        <div>
          ...
          <Row>
            <Col xs="128" md="10">
            <Download link="rpjmd/menyusun/proyeksi-keuangan"/>
        ...
        <Row>
          <Col xs="12" lg="12">
            <Card>
              ...
              <PaguIndikatifCreate/>
              <AnalisisKeuanganCreate/>
              <CardBody>
                
                <h5 style={{textAlign: 'center',}}>PROYEKSI KEMAMPUAN KEUANGAN DAERAH</h5>
                <hr/>
...
export default ProyeksiKeuanganCreate;
