...
class VisiPenjelasan extends Component {
  ...
  constructor(props) {
    ...
    this.dataPilihAwal = {
      misi_penjelasan_id:0,
      misi_penjelasan_nama:'',
    };
  ...
  handleChange(event){
    ...
    if(parseInt(no)===1){
      setData.misi_penjelasan_nama = event.target.value;
    }
...
/// set data
  ...
  getData = (page = 1) => {
    ...
    axios
    .post(config.apiRoot+'rpjmd/menyusun/misi-penjelasan/page-'+page, data)
    .then(response => {
...
// submit
  handleSubmit = event => {
    ...
    if(this.state.aksi === 'Edit'){
      link = config.apiRoot+'rpjmd/menyusun/misi-penjelasan/update';
      
    }else if(this.state.aksi === 'Tambah'){
      link = config.apiRoot+'rpjmd/menyusun/misi-penjelasan/create';
    }
    ...
    data.append('session', localStorage.getItem('codexv-session'));
    data.append('rpjmd', localStorage.getItem('codexv-rpjmd'));
    data.append('misi_penjelasan_nama', this.state.dataPilih.misi_penjelasan_nama);
    axios
    .post(link, data, head)
    .then(response => {
                    ...
                    <Form method="post"  onSubmit={this.handleSubmit} className="form-horizontal" id="form-data">
                      <ModalBody>
                        <FormGroup row>
                          ...
                          <Col xs="12" md="9">
                            <Input type="text" data-number="1" placeholder="" onChange={this.handleChange} value={this.state.dataPilih.misi_penjelasan_nama} required autoFocus/>
                            <FormText color="muted">Isi Penjelasan Misi</FormText>
                          </Col>
                        </FormGroup>
                      </ModalBody>
                      <ModalFooter>
