...
class AdminUsulanPokir extends Component {
        ...
        <div>
          ...
          <Table responsive striped>
            ...
            {this.state.dataKiriman.map((data, key) => {
              return data ? (
                <tr key={key}>
                <td>{"Kecamatan "+data.kecamatan+", Kelurahan "+data.kelurahan}</td>
                <td>{data.nama}</td>
                <td>{data.alasan}</td>
                <td>{data.lokasi}</td>
                <td>{data.volume}</td>
  ...
  downloadGambar = (data) =>{
    ...
    return (
      <Form method="post" id="download-gambar" action={config.apiRoot+'attachments/foto-'+asal+'/'+data.file} target="_blank">
        <img type="submit" form="download-gambar" src={config.apiRoot+'attachments/foto-'+asal+'/'+data.file} style={{height:'100px'}} /><br/>
        <Input  type="hidden" name="session" value={localStorage.getItem('codexv-session')}/>
  ...
  getKiriman = (page = 1) => {
    ...
    data.append('search', this.state.pencarian);
    axios
    .post(config.apiRoot+'musrenbang/kiriman-pokir/page-'+page, data)
    .then(response => {
      if(response.data.status){
  ...
  handleKecamatanSubmit = (event)=>{
    ...
    axios
    .post(config.apiRoot+'musrenbang/kiriman-pokir/page-'+1, data)
    .then(response => {
      if(response.data.status){
...
export default AdminUsulanPokir;
