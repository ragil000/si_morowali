...
class MyConfig {
    ...
    public function check($session, $level, $cekStatus = false){
		$status = false;
		$codeSession = $session;
        ...
        if($cekStatus){
            ...
            if($arrSession['status'] && $arrSession['level'] == $level){
                $this->header($arrSession['domain']);
                $status = true;
            }
        }
    ...
    public function getSession($session, $level = 1, $cekStatus = false){
        $status = false;
