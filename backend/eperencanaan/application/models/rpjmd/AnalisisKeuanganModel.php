...
class AnalisisKeuanganModel extends CI_Model
{
    ...
    public function __construct()
    {
        parent::__construct();
        $this->jumlah = 20;
        $this->table = 'ta_analisis_keuangan';
    }

    public function getCount($search = '', $post){

        $this->db->join('ref_rpjmd', 'ref_rpjmd.rpjmd_id = ta_analisis_keuangan.rpjmd_id', 'left');
    ...
    public function getAll($page = 1, $search = '', $post){
        $jumlah = $this->jumlah;
        $awal = ($page - 1)*$jumlah;
        $this->db->join('ref_rek_4', 'ref_rek_4.Kd_Rek_1 = ta_analisis_keuangan.Kd_Rek_1 AND ref_rek_4.Kd_Rek_2 = ta_analisis_keuangan.Kd_Rek_2 AND ref_rek_4.Kd_Rek_3 = ta_analisis_keuangan.Kd_Rek_3 AND ref_rek_4.Kd_Rek_4 = ta_analisis_keuangan.Kd_Rek_4 ', 'left');
        $this->db->join('ref_rpjmd', 'ref_rpjmd.rpjmd_id = ta_analisis_keuangan.rpjmd_id', 'left');
        $this->db->join('ref_rpjmd_user', 'ref_rpjmd_user.rpjmd_id = ref_rpjmd.rpjmd_id', 'left');
        $this->db->where('ref_rpjmd_user.user_id', $post['user_id']);
        $this->db->where('ref_rpjmd_user.rpjmd_id', $post['rpjmd']);
    ...
    public function update($post){
        ...
        if($data > 0){
            ...
            $id = $post['analisis_keuangan_id'];
            $this->db->where('analisis_keuangan_id', $id);
            $data = array(
                'Kd_Rek_1' => $rek[0], 
                'Kd_Rek_2' => $rek[1], 
                'Kd_Rek_3' => $rek[2], 
                'Kd_Rek_4' => $rek[3], 
                'data_tahun_dasar' => $post['data_tahun_dasar'], 
                'tingkat_pertumbuhan' => $post['tingkat_pertumbuhan'], 
                'tahun1' => $post['tahun1'], 
                'tahun2' => $post['tahun2'], 
                'tahun3' => $post['tahun3'], 
                'tahun4' => $post['tahun4'], 
