...
class RefHspkController extends CI_Controller {
    ...
    {
        parent::__construct();
        $this->load->model('refHspk/RefHspkModel');
	...
	public function index($page = 1,$dataId = null)
	{
		...
		$data['page'] = $page;
		$data['jumlahSshInPage'] = $this->RefHspkModel->getJumlahInPage();
		$data['jumlahSsh'] = $this->RefHspkModel->getCount($search);
		$data['search'] = $search;
		...
		$dataTemplate['isi'] = $this->load->view('refHspk/ref-hspk/table', $data, true);
		$dataTemplate['myScript'] = $this->load->view('refHspk/ref-hspk/script.php', null, true);
