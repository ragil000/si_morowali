...
class RefSsh1Controller extends CI_Controller {
    ...
    {
        parent::__construct();
        $this->load->model('refSsh/RefSsh1Model');
	...
	public function index($page = 1,$dataId = null)
	{
		...
		$data['page'] = $page;
		$data['tableName'] = 'ssh1';   //for page
		$data['jumlahRecInPage'] = $this->RefSsh1Model->getJumlahSshInPage();
		$data['jumlahRec'] = $this->RefSsh1Model->getCountSsh($search);
		$data['search'] = $search;

		$data['ssh'] = $this->RefSsh1Model->getAllSsh($page, $search);

		$dataTemplate['isi'] = $this->load->view('refSsh/ref-ssh1/table', $data, true);
		$dataTemplate['navigator'] = $this->load->view('include/navigator', $data, true);
		$dataTemplate['myScript'] = $this->load->view('refSsh/ref-ssh1/script.php', null, true);
	...
	public function deleteSsh($dataId){
		$pesan = '';
		if($this->RefSsh1Model->deleteSsh($dataId)){
	...
	public function getSsh($id,$tableName){
		$data = $this->RefSsh1Model->getSsh($id,$tableName);
