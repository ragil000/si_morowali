	...
	<script type="text/javascript">
		...
		function createSsh(obj){
			...
			var dataSsh = 1;
			getSsh(dataSsh, id);
			$('#view-title').html('Tambah SSH Zona <?=$zona?>');
		...
		function updateSsh(obj){
			...
			$('#view-title').html('Edit SSH Zona <?=$zona?>');
		...
		function getDataSsh(id, view = true){
			var url = link+'admin/ssh/load-data-ssh/<?=$zona?>';
			$.ajax({
	           type: "POST",
	           url: url,
	           dataType: "JSON",
	           ...
	           success: function(data)
	           {
	           		...
	           		//tampilan(data);
	           		if(view){
									var x=convertToRupiah(data['harga_zona<?=$zona?>']);
	           			$('#view-kode-ssh').html(": "+data['Kd_Ssh1']+"."+data['Kd_Ssh2']+"."+data['Kd_Ssh3']+"."+data['Kd_Ssh4']+"."+data['Kd_Ssh5']+"."+data['Kd_Ssh6']);
		           		$('#view-kode-ssh1').html(": "+data['Nm_Ssh1']);
		           		$('#view-kode-ssh2').html(": "+data['Nm_Ssh2']);
		           		$('#view-kode-ssh3').html(": "+data['Nm_Ssh3']);
		           		$('#view-kode-ssh4').html(": "+data['Nm_Ssh4']);
		           		$('#view-kode-ssh5').html(": "+data['Nm_Ssh5']);
		           		$('#view-kode-ssh6').html(": "+data['Kd_Ssh6']);
		           		$('#view-barang').html(": "+data['Nama_Barang']);
									if (data['Satuan']==null)
										$('#view-satuan').html(": - ");
									else
										$('#view-satuan').html(": "+data['Satuan']);
		           		$('#view-harga').html(": "+x);
	           		}else{
	           			$('#kode-ssh').val(data['Kd_Ssh1']+"-"+data['Kd_Ssh2']+"-"+data['Kd_Ssh3']+"-"+data['Kd_Ssh4']+"-"+data['Kd_Ssh5']+"-"+data['Kd_Ssh6']);
	           			for(let i = 1; i<=5;i++){
	           				$('#kode-ssh'+i).attr('disabled', true);
	           				$('#kode-ssh'+i).empty().append('<option value="'+data['Kd_Ssh'+i]+'">'+data['Nm_Ssh'+i]+'</option>');
	           			}
	           			$('#kode-ssh6').attr('disabled', true);
	           			...
	           			$('#harga-zona-<?=$zona?>').val(data['harga_zona<?=$zona?>']);
	           		}
