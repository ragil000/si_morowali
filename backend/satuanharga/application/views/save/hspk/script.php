	...
	<script type="text/javascript">
		...
		function isiTable(data, baru = true, table = true, id = null){
			...
			if(data){
				...
				for (let i = 0; i < data.length; i++) {
					...
					if(id != null || !table){
						isiTable = '<tr><td></td><td>'+nomor+'</td><td>'+uraian+'</td><td>'+koefisien+'</td><td>'+hargaSatuan+'</td><td>'+satuan+'</td><td>'+roundToTwo(harga)+'</td></tr>';
					}else{
					...
					if(dataUpah.length > 0){
						...
						$('#'+nameTable+' > tbody').append('<tr><td></td><td colspan="4"></td><td>Total Upah</td><td>Rp '+roundToTwo(totalUpah)+'</td></tr>');
					}
					...
					if(dataBahan.length > 0){
						...
						$('#'+nameTable+' > tbody').append('<tr><td></td><td colspan="4"></td><td>Total Bahan</td><td>Rp '+roundToTwo(totalbahan)+'</td></tr>');
					}
					if(dataSewa.length > 0){
						...
						$('#'+nameTable+' > tbody').append('<tr><td></td><td colspan="4"></td><td>Total Sewa</td><td>Rp '+roundToTwo(totalSewa)+'</td></tr>');
					}
			...
			$('#harga-zona-<?=$zona?>').val(roundToTwo(total));
			//alert(total);
	  ...
	  //       console.log(dontDataHspk);
        ...
        setTimeout(function() {
            <?php 
                foreach ($ssh as $data){
                    echo "viewTable('".$data['Kd_Hspk1']."-".$data['Kd_Hspk2']."-".$data['Kd_Hspk3']."-".$data['Kd_Hspk4']."');";
                    echo "console.log('".$data['Kd_Hspk1']."-".$data['Kd_Hspk2']."-".$data['Kd_Hspk3']."-".$data['Kd_Hspk4']."');";
                }
            ?>
		}, 5000);
