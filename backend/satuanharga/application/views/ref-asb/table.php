			...
			<div class="min-height-200px">
				...
				<div class="pd-20 bg-white border-radius-4 box-shadow mb-30">
					<div class="clearfix mb-1">
						<div class="pull-left">
							<h4 class="text-blue">ASB Zona <?=$zona?></h4>
						</div>
						<div class="pull-right">
							<form action="<?=base_url()?>admin/asb/page-1">
								<div class="form-group has-success">
									<input type="text" class="form-control form-control-success" name="search" placeholder="Pencarian Jenis Pekerjaan" value="<?=$search?>" size="25">
								</div>
					...
					<div class="clearfix mb-10">
						<div class="pull-left btn-group btn-group2">
							<a class=	"<?=($zona == 1) ? 'btn btn-primary disabled' : 'btn btn-primary';?>"
								href="<?=base_url()?>admin/asb/page-<?=$page?>?search=<?=$search?>&zona=1">Zona 1</a>
							<a class= "<?=($zona == 2) ? 'btn btn-warning disabled' : 'btn btn-warning';?>"
							  href="<?=base_url()?>admin/asb/page-<?=$page?>?search=<?=$search?>&zona=2">Zona 2</a>
							<a class= "<?=($zona == 3) ? 'btn btn-primary disabled' : 'btn btn-primary';?>"
							  href="<?=base_url()?>admin/asb/page-<?=$page?>?search=<?=$search?>&zona=3">Zona 3</a>
							<a class= "<?=($zona == 4) ? 'btn btn-primary disabled' : 'btn btn-primary';?>"
							  href="<?=base_url()?>admin/asb/page-<?=$page?>?search=<?=$search?>&zona=4">Zona 4</a>
						</div>
						<div class="pull-right">
							...
							<tbody>
								<?php
									foreach ($ssh as $data) {
										...
										echo "
								<tr>
									<td><a href='javascript:void(0)' data-id='".$data['Kd_Asb1']."-".$data['Kd_Asb2']."-".$data['Kd_Asb3']."-".$data['Kd_Asb4']."-".$data['Kd_Asb5']."' onclick='viewTable(this)'><i class='fa fa-caret-square-o-down'></i></a></td>
								...
								<tr  style='background-color: #ffffff;' >
									<td id='table-view-".$data['Kd_Asb1']."-".$data['Kd_Asb2']."-".$data['Kd_Asb3']."-".$data['Kd_Asb4']."-".$data['Kd_Asb5']."' style='display: none;'>
										<table class='table table-bordered' id='tableTampil-".$data['Kd_Asb1']."-".$data['Kd_Asb2']."-".$data['Kd_Asb3']."-".$data['Kd_Asb4']."-".$data['Kd_Asb5']."'>
											<thead>
												<tr>
													<th></th>
													<th>Nomor</th>
