			...
			if(data){
				...
				for (let i = 0; i < data.length; i++) {
					...
					if(data[i]['Kategori_Pekerjaan'] != tempKategori ){
						if(tempKategori != 0){
							$('#'+nameTable+' > tbody').append('<tr><td colspan="5"></td><td>Total</td><td>'+converToRupiah(totalKategori)+'</td></tr>');
							totalKategori = 0;	
						}
					...
					if(table)
						...
						$('#'+nameTable+' > tbody').append('<tr><td></td><td>'+nomor+'</td><td>'+kategori+'</td><td>'+koefisien+'</td><td>'+converToRupiah(hargaSatuan)+'</td><td>'+satuan+'</td><td>'+converToRupiah(harga)+'</td></tr>');
					...
					if(tulisTotal){
						$('#'+nameTable+' > tbody').append('<tr><td colspan="5"></td><td>Total</td><td>'+totalKategori+'</td></tr>');
						totalKategori = 0;						
					}
				...
				$("#"+nameTable+" > tfoot th:eq(" + 6 + ")").html("Rp. "+converToRupiah(total));
