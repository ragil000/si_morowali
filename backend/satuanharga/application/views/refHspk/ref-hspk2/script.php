	...
	<script type="text/javascript">
		...
		function updateHspk(obj){
			...
			setComponent(false);
			$('#kode-hspk2').attr('type', 'text').val(arr_id[1]);
			$('#nm-hspk2').attr('type', 'text').val(arr_str[1]);
			...
			var s='Data <strong><i>"'+uraian+'"</i></strong> akan dihapus ?';
					swal({
							...
							if (dismiss['value']) {
								...
								window.location=link+"admin/hspk2/"+id;
							}else{
								swal(
											'Batal!',
											'Data batal dihapus',
		...
		function getHspk(id,tableName,status){ //display all table with any categories
			...
			$.ajax({
				type:'GET',
				url:link+"admin/hspk2/get-hspk/"+id+"/"+tableName,
				dataType: "JSON",
				success:function(data){
					if(data){
						if (tableName=="ref_hspk2" && status=="last record"){
							 let x=parseInt(data[0]['Kd_Hspk2'])+1; //last record+1 to insert rec
							 $('#kode-hspk2').attr('type', 'text').val(x);//kd_hspk2
						}
			...
			if (x=='' && tableName=="ref_hspk2" && status=="last record")//if data is null, default 1
				 $('#kode-hspk2').attr('type', 'text').val('1');
		}
		...
		$("#kode-hspk1").change(function () {    //change combo ref_hspk1
			...
			if (id == ''){
					...
					$('#nm-hspk2').attr("disabled", true);
					$('#kode-hspk2').attr('type', 'text').val('');
					$('#nm-hspk2').attr('type', 'text').val('');
			}
			else {
					$('#kode-hspk2').attr("disabled", false);
					$('#nm-hspk2').attr("disabled", false);
					getHspk(id,'ref_hspk2','last record');
			}
		});
		...
		function setComponent(active) {
			...
			$('#nm-hspk2').attr("disabled", active);
			$('#kode-hspk1').attr('type', 'text').val('');
			$('#kode-hspk2').attr('type', 'text').val('');
			$('#nm-hspk2').attr('type', 'text').val('');
			if (active) { //create
				$('#view-title').html('Tambah Kode HSPK 2');
				$('#hspk-submit').attr('type', 'submit').val('Buat HSPK');
			}else{  //update
				$('#view-title').html('Edit Kode HSPK 2');
