		...
		function isiTable(data, baru = true, table = true){
			...
			if(data){
				...
				for (let i = 0; i < data.length; i++) {
					...
					let hargaSatuan = data[i]['Harga_Satuan'];
					let satuan = data[i]['Kd_Satuan'];
					let harga = hargaSatuan*koefisien;
					total += harga;
					$('#'+nameTable+' > tbody').append('<tr><td><a href="javascript:void(0)" data-hspk="'+kodeHspk+'" data-uid="'+kode+'" onclick="deleteInTable(this)"><i class="fa fa-trash" aria-hidden="true"></i></a></td><td>'+nomor+'</td><td>'+kategori+'</td><td>'+koefisien+'</td><td>'+hargaSatuan+'</td><td>'+satuan+'</td><td>'+harga+'</td></tr>');

					

				}

				$("#"+nameTable+" > tfoot th:eq(" + 6 + ")").html(total);
			...
			var id= $(obj).attr('data-uid');
           	swal({
                ...
                if (dismiss['value']) {
                	...
                	window.location=link+"admin/hspk/"+id;
